...
assert.ok(patch.includes('details class="fingerprint-group"'),'aroma descriptors are not folded under their family');
assert.ok(app.includes('metric-info'),'metric ranges are not available behind an information control');
assert.ok(app.includes('metric-score-line'),'metric information control is not aligned with the score');
assert.ok(!app.includes('Plage typique ${range.replace'),'metric popover still repeats the redundant range label');
